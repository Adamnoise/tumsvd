...
export const ExportTab: React.FC<ExportTabProps> = ({ state, pathData }) => {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      {/* Advanced Export Modal Button */}
      <button
        onClick={() => setShowExportModal(true)}
        className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 text-sm font-medium hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
      >
        <Code2 className="w-4 h-4" aria-hidden="true" />
        Export Code (CSS, React, Vue)
      </button>
